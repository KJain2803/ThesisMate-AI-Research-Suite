...
import os
st.set_page_config(
    page_title="ThesisMate AI Research Suite",
    page_icon="🧠",
    layout="wide"
)
saved_graphs = []
...
def save_matplotlib_graph(x, y, title, xlabel, ylabel, graph_name):
    ...
    file_path = f"{graph_name}.png"
    plt.tight_layout()
    plt.savefig(file_path, dpi=220)
    plt.close()
    saved_graphs.append(file_path)
...
def analyze_dissertation_text(text):
    ...
    methodology_words = ["method", "media", "treatment", "culture", "analysis", "design", "procedure"]

    methodology_hits = sum([1 for word in methodology_words if word.lower() in text.lower()])
    ...
    if methodology_score < 60:
        suggestions.append("Methodology explanation requires stronger scientific detailing.")
...
with main_tab1:
    ...
    if doc_file is not None:
        ...
        st.text_area(label="Dissertation Preview", value=dissertation_text[:1500], height=180)
        

        writing_score, citation_strength, methodology_score, suggestions = analyze_dissertation_text(dissertation_text)
...
with main_tab2:
    ...
    data_mode = st.selectbox("Choose Data Input Mode", ["Upload Excel File", "Upload CSV File", "Manual Data Entry"])

    data_file = None
    df = None

    if data_mode == "Upload Excel File":
        data_file = st.file_uploader("Upload Experimental Dataset (.xlsx)", type=["xlsx"])
    ...
    if data_mode == "Manual Data Entry":
        ...
        df = st.data_editor(pd.DataFrame(columns=["Treatment","Days","Response"]), num_rows="dynamic")
    ...
    if data_file is not None:
        ...
        numeric_cols = [col for col in df.select_dtypes(include='number').columns.tolist() if col != "Days"]
    ...
    if run_analysis:
            ...
            if doc_file is not None:
                ...
                st.warning("Recent literature comparison appears limited in uploaded chapter.")
                ...
                if word_count < 1500:
                    ...
                    st.write("• Academic depth appears acceptable but transitions can improve.")

                if citation_strength == "Low":
                    st.write("• Add more recent citation-backed references.")
                else:
                    st.write("• Citation support is present but comparative literature can increase.")

                if methodology < 75:
                    st.write("• Methodology explanation requires stronger scientific detailing.")
                else:
            ...
            if data_file is not None:
                ...
                if "Days" in df.columns and test_choice in ["Full Automatic Suite", "One Way ANOVA"]:
                    grouped = df.groupby("Days")[y_axis].mean().reset_index()
                    save_matplotlib_graph(grouped["Days"], grouped[y_axis],
                        "ANOVA Mean Response Across Days",
                        "Days", y_axis, "anova_graph")
                    ...
                    if p_val < 0.05:
                        ...
                        significance = "Statistically significant group difference detected."
                    else:
                        st.warning("No statistically significant group difference detected.")
                        significance = "No statistically significant group difference detected."
            ...
            """
                    ...
                    fig_scatter = px.scatter(df, x="Days", y=y_axis,)
                    ...
                    err_df = df.groupby("Days")[y_axis].agg(['mean','std','count']).reset_index()
                    err_df['se'] = err_df['std'] / np.sqrt(err_df['count'])

                    fig_err = px.line(err_df, x="Days", y="mean", markers=True, error_y="se")
                    ...
                    if "Treatment" in df.columns:
                        ...
                        pivot = df.pivot_table(values=y_axis, index="Treatment", columns="Days", aggfunc='mean')
                        ...
                        if "Treatment" in df.columns:
                            ...
                            treat_mean = df.groupby("Treatment")[y_axis].mean().reset_index()
                            fig_donut = px.pie(treat_mean, names="Treatment", values=y_axis, hole=0.5)
                            ...
                            plt.tight_layout()
                            plt.savefig("graph_donut.png")
                            plt.close()
                            saved_graphs.append("graph_donut.png")
                        if "Treatment" in df.columns:
                            st.subheader("🕸️ Multi-Treatment Radar Intelligence")
                            radar = df.groupby("Treatment")[y_axis].mean().reset_index()
                            fig_radar = px.line_polar(radar, r=y_axis, theta="Treatment", line_close=True)
                        ...
                        performance_percent = min(100, round((avg_perf/max_perf)*100))
                        ...
                        st.dataframe(top_obs, use_container_width=True)
                    ...
                    if test_choice == "Full Automatic Suite":
                        ...
                        st.write(f"**Pearson Correlation (Days vs {y_axis}) = {corr_value}**")

                        if corr_value > 0:
                            st.success("Positive correlation detected: response variable increases with culture duration.")
                        else:
                            st.warning("Negative/weak correlation trend detected.")

                            
                        # Regression
                        slope, intercept, r_value, p_reg, std_err = stats.linregress(df["Days"], df[y_axis])

                        st.write(f"**Linear Regression Equation:** {y_axis} = {round(intercept,3)} + {round(slope,3)}(Days)")
                        ...
                        plt.title("Frequency Distribution Histogram", fontsize=13, fontweight="bold", color="#123A63")
                        plt.xlabel(y_axis)
                        plt.ylabel("Frequency")
                        plt.grid(axis='y', linestyle='--', alpha=0.3)
                        plt.tight_layout()
                        plt.savefig("graph_hist.png", dpi=220)
                        plt.close()
                        saved_graphs.append("graph_hist.png")

                        # Treatment Ranking
                        st.subheader("🏆 Treatment Performance Ranking")
                        treatment_col = "Treatment" if "Treatment" in df.columns else df.columns[0]
                        ranking = df.groupby(treatment_col)[y_axis].mean().sort_values(ascending=False).reset_index()
                        st.dataframe(ranking, use_container_width=True)

                        top_treatment = ranking.iloc[0][treatment_col]
                        top_value = round(ranking.iloc[0][y_axis],3)

                        st.info(f"Top ranked treatment based on mean {y_axis} = {top_treatment} ({top_value})")
                        plt.figure(figsize=(7,4.5))
                        bars = plt.bar(ranking["Treatment"], ranking[y_axis], color="#3498DB", edgecolor="black")
                        plt.title("Treatment Performance Ranking", fontsize=13, fontweight="bold", color="#123A63")
                        ...
                        plt.tight_layout()
                        plt.savefig("graph_treatment.png", dpi=220)
                        plt.close()
                        saved_graphs.append("graph_treatment.png")
                        ...
                        confidence_score = min(99, round((abs(corr_value)*30) + ((r_value**2)*30) + ((100-cv)/2)))
                        novelty_score = min(98, round((best_mean*5) if isinstance(best_mean,(int,float)) else 85))
                        ...
                        st.success("AI Verdict: Dataset demonstrates publishable analytical strength with statistically interpretable treatment differentiation.")
                        ...
                        def add_page_number(canvas, pdf_doc):
                            ...
                            canvas.drawString(40, 20, "Generated by ThesisMate AI Research Engine™")
                            canvas.drawRightString(550, 20, f"Page {pdf_doc.page}")
                            canvas.restoreState()
                        pdf_buffer = io.BytesIO()
                        pdf_doc = SimpleDocTemplate(pdf_buffer, pagesize=A4,rightMargin=30,leftMargin=30,topMargin=30,bottomMargin=30)
                        ...
                        cover_box = Table([
                            [Paragraph("<b>THESISMATE AI EXECUTIVE RESEARCH DOSSIER</b>", title_style)],
                            [Paragraph("Automated Scientific Intelligence | Dissertation Analytics | Biostatistical Modelling", sub_style)],
                            [Paragraph(f"Target Variable Analysed: <b>{y_axis}</b>", styles["Heading3"])],
                            [Paragraph("Generated on ThesisMate AI Research Engine™", styles["Heading3"])],
                            [Paragraph("CONFIDENTIAL SCIENTIFIC ANALYTICS REPORT", styles["Heading2"])]
                        ], colWidths=460)

                        cover_box.setStyle(TableStyle([
                            ('BOX',(0,0),(-1,-1),2,colors.HexColor("#0A2A66")),
                            ('BACKGROUND',(0,0),(-1,-1),colors.HexColor("#EDF4FB")),
                            ('ALIGN',(0,0),(-1,-1),'CENTER'),
                            ('VALIGN',(0,0),(-1,-1),'MIDDLE'),
                            ('PADDING',(0,0),(-1,-1),22),
                            ('GRID',(0,0),(-1,-1),0.5,colors.HexColor("#9CC2E5"))
                        ]))
                        story.append(cover_box)
                        story.append(PageBreak())

                        # -------- KPI DASHBOARD PAGE ----------
                        story.append(Paragraph("AI SCIENTIFIC EXECUTIVE SUMMARY", sub_style))
                        ...
                        kpi_table.setStyle(TableStyle([
                            ('GRID',(0,0),(-1,-1),1,colors.HexColor("#0A2A66")),
                            ('BACKGROUND',(0,0),(-1,-1),colors.whitesmoke),
                            ('TEXTCOLOR',(0,0),(-1,-1),colors.HexColor("#0A2A66")),
                            ('PADDING',(0,0),(-1,-1),10)
                        ]))
                        ...
                        verdict_table = Table([[Paragraph(verdict, normal_style)]], colWidths=430)
                        ...
                        sections = [
                            ...
                            ["STATISTICAL REVIEW", f"Analyzed Variable: {y_axis}<br/>F-value: {round(f_val,3)}<br/>P-value: {round(p_val,3)}<br/>Conclusion: {significance}<br/>Best Group: Days {best_day} with mean {round(best_mean,3)}"],
                            ["AI SCIENTIFIC INTERPRETATION", interpretation],
                            ["RESULT WRITER PARAGRAPH", result_writer]
                        ]

                                                # -------- EXECUTIVE SCIENTIFIC METRIC DASHBOARD --------
                        story.append(Paragraph("EXECUTIVE SCIENTIFIC METRIC DASHBOARD", sub_style))
                        story.append(Spacer(1,18))

                        metric_data = [
                            [
                                Paragraph(f"<b>F STATISTIC</b><br/>{round(f_val,3)}", styles["BodyText"]),
                                Paragraph(f"<b>P VALUE</b><br/>{round(p_val,4)}", styles["BodyText"])
                            ],
                            [
                                Paragraph(f"<b>PEARSON CORRELATION</b><br/>{corr_value}", styles["BodyText"]),
                                Paragraph(f"<b>REGRESSION R²</b><br/>{round(r_value**2,3)}", styles["BodyText"])
                            ],
                            [
                                Paragraph(f"<b>COEFF. OF VARIATION</b><br/>{cv}%", styles["BodyText"]),
                                Paragraph(f"<b>BEST TREATMENT</b><br/>{top_treatment}", styles["BodyText"])
                            ]
                        ]

                        metric_table = Table(metric_data, colWidths=[225,225], rowHeights=[72,72,72])
                        metric_table.setStyle(TableStyle([
                            ('BOX',(0,0),(-1,-1),1.5,colors.HexColor("#1F4E79")),
                            ('GRID',(0,0),(-1,-1),1,colors.HexColor("#9CC2E5")),
                            ('BACKGROUND',(0,0),(-1,-1),colors.HexColor("#EDF4FB")),
                            ('VALIGN',(0,0),(-1,-1),'MIDDLE'),
                            ('ALIGN',(0,0),(-1,-1),'CENTER'),
                            ('FONTSIZE',(0,0),(-1,-1),11),
                            ('TEXTCOLOR',(0,0),(-1,-1),colors.HexColor("#123A63")),
                            ('FONTNAME',(0,0),(-1,-1),'Helvetica-Bold')
                        ]))
                        story.append(metric_table)
                        story.append(Spacer(1,25))
                        for heading, content in sections:
                            head = Table([[Paragraph(f"<b>{heading}</b>", styles["Heading3"])]], colWidths=450)
                            head.setStyle(TableStyle([
                                ('BACKGROUND',(0,0),(-1,-1),colors.HexColor("#D9EAF7")),
                                ('BOX',(0,0),(-1,-1),1,colors.HexColor("#0A2A66")),
                                ('PADDING',(0,0),(-1,-1),8)
                            ]))
                            story.append(head)

                            body = Table([[Paragraph(content.replace("\n","<br/>"), normal_style)]], colWidths=450)
                        ...
                        story.append(PageBreak())
                        desc = df[[col for col in df.select_dtypes(include='number').columns]].describe().round(2)
                        ...
                        desc_table.setStyle(TableStyle([
                            ...
                            ('TEXTCOLOR',(0,0),(-1,0),colors.HexColor("#0A2A66")),
                            ('FONTSIZE',(0,0),(-1,-1),7),
                            ('PADDING',(0,0),(-1,-1),4)
                        ]))
                        ...
                        story.append(Spacer(1,20))
                        # -------- STAT TABLES ----------
                        story.append(Paragraph("DETAILED STATISTICAL DATA ANNEXURE", sub_style))
                        ...
                        if "Treatment" in df.columns:
                            rank_df = df.groupby("Treatment")[y_axis].mean().round(3).reset_index()
                        ...
                        raw_cols = [col for col in [treatment_col, "Days", y_axis] if col in df.columns]
                        raw_df = df.sort_values(by=y_axis, ascending=False)[raw_cols].head(10).round(3)
                        ...
                        # ---------- GRAPHICAL EVIDENCE ----------
                        story.append(Paragraph("AI GRAPHICAL SCIENTIFIC EVIDENCE", sub_style))
                        story.append(Spacer(1,20))
                        ...
                        for g in saved_graphs:
                            ...
                            if not os.path.exists(g):
                                continue
                            graph_title = g.replace(".png","").replace("_"," ").upper()

                            graph_head = Table([[Paragraph(graph_title, styles["Heading4"])]], colWidths=450)
                            graph_head.setStyle(TableStyle([
                            ...
                            img = Image(g, width=430, height=240)
                            story.append(img)
                            story.append(Spacer(1,8))

                            comment = graph_comments.get(g, "Graphical statistical evidence generated through ThesisMate analytical engine.")
                            cbox = Table([[Paragraph(comment, styles["BodyText"])]], colWidths=450)
                        ...
                        story.append(PageBreak())
                        story.append(Paragraph("THESISMATE AI SCIENTIFIC CERTIFICATION", sub_style))
                        story.append(Spacer(1,25))

                        cert_data = [
                            [Paragraph(f"<b>STATISTICAL CONFIDENCE SCORE</b><br/>{confidence_score}%", styles["BodyText"])],
                            [Paragraph(f"<b>RESEARCH STRENGTH INDEX</b><br/>{novelty_score}%", styles["BodyText"])],
                            [Paragraph(f"<b>TOP PERFORMING TREATMENT</b><br/>{top_treatment}", styles["BodyText"])],
                            [Paragraph("<b>PUBLISHABILITY VERDICT</b><br/>Dataset demonstrates strong scientific interpretability, reproducible analytical depth and dissertation-grade publication readiness under AI-assisted evaluation.", styles["BodyText"])]
                        ]
                        ...
                        story.append(Paragraph("Certified and Generated by ThesisMate AI Research Engine™", styles["Heading3"]))
                        story.append(Spacer(1,10))
                        story.append(Paragraph("Automated Dissertation Intelligence • Biostatistical Validation • Scientific Writing Synthesis", styles["BodyText"]))
                        story.append(Spacer(1,20))
                        story.append(Paragraph("<b>----- END OF AI SCIENTIFIC DOSSIER -----</b>", styles["Heading3"]))
            ...
            if test_choice == "Pearson Correlation":
                st.markdown("---")
                ...
                if "Days" in df.columns:
                    ...
                    fig_corr = px.scatter(df, x="Days", y=y_axis,)
                    st.plotly_chart(fig_corr, use_container_width=True)
                    save_matplotlib_graph(df["Days"], df[y_axis],
                                  "Pearson Correlation Scatter",
                                  "Days", y_axis, "pearson_graph")

                    if corr_value > 0:
                        st.success("Positive correlation detected between Days and response variable.")
                    else:
                        st.warning("Negative or weak relationship detected.")

            if test_choice == "Linear Regression":
                st.markdown("---")
                ...
                if "Days" in df.columns:
                    ...
                    st.write(f"Regression Equation: {y_axis} = {round(intercept,3)} + {round(slope,3)}(Days)")
                    st.write(f"R² = {round(r_value**2,3)}")
                    fig_reg = px.scatter(df, x="Days", y=y_axis,)
                    st.plotly_chart(fig_reg, use_container_width=True)
                    save_matplotlib_graph(df["Days"], df[y_axis],
                                  "Linear Regression Model",
                                  "Days", y_axis, "regression_graph")

            if test_choice == "CRD Treatment Ranking":
                st.markdown("---")
                st.header("🏆 CRD Treatment Ranking Module")

                treatment_col = st.selectbox("Select Treatment Column for Ranking", df.columns)

                ranking = df.groupby(treatment_col)[y_axis].mean().sort_values(ascending=False).reset_index()
                st.dataframe(ranking, use_container_width=True)
                fig_rank = px.bar(ranking, x=treatment_col, y=y_axis, text_auto=True)
                st.plotly_chart(fig_rank, use_container_width=True)
                save_matplotlib_graph(ranking[treatment_col], ranking[y_axis],
                                    "CRD Treatment Ranking",
                              treatment_col, y_axis, "crd_graph")
...
with main_tab4:
    if run_analysis and pdf_buffer is not None and test_choice == "Full Automatic Suite":
        st.markdown("<div class='section-card'>", unsafe_allow_html=True)
